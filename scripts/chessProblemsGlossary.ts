...
const OUT_FILE = path.join(__dirname, "Data", "structured", "chess_problems_glossary.json");

(async () => {
    const browser = await puppeteer.launch({ headless: true });
    const page = await browser.newPage();
    console.log("🌐 Navigating to Wikipedia Chess Problems Glossary...");
    await page.goto("https://en.wikipedia.org/wiki/Glossary_of_chess_problems", { waitUntil: "networkidle0" });
    ...
    const structured = raw.map(r => ({
        ...
        category: "problems",
        source: "chess_problems_glossary",
        text: `Term: ${r.term}\nDefinition: ${r.definition}`,
    }));

    fs.mkdirSync(path.dirname(OUT_FILE), { recursive: true });
    fs.writeFileSync(OUT_FILE, JSON.stringify(structured, null, 2));
    console.log(`✅ Saved ${structured.length} problem glossary terms → ${OUT_FILE}`);
    await browser.close();
})();